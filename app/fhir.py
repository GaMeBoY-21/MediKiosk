...
def build_fhir_bundle(
    ...
    def _condition(label: str, key: str, category: str, is_active: bool = True) -> None:
        ...
        condition_refs.append({"reference": _urn(cid)})

    if summary.chief_complaint:
        _condition(summary.chief_complaint, "chief", "encounter-diagnosis")
    for i, item in enumerate(history.get("past_medical") or []):
        _condition(item, f"past-{i}", "problem-list-item")
    ...
    for i, med in enumerate(history.get("medications") or []):
        mid = _uid(patient_id, f"medication:{i}")
            ...
            _entry(
                {
                    ...
                    "id": mid,
                    "status": "active",
                    "medicationCodeableConcept": _text(med),
                    "subject": patient_ref,
    ...
    # ------------------------------------------------------- Observations
    # Built from document findings, which are genuinely structured: label,
    # value, unit, reference range and an out-of-range flag.
    ...
    for doc in summary.document_timeline or []:
        for i, finding in enumerate(doc.findings or []):
            oid = _uid(patient_id, f"observation:{doc.doc_id}:{i}")
            resource: Dict[str, Any] = {
                "resourceType": "Observation",
                "id": oid,
                "status": "final" if verified else "preliminary",
                "category": [
                    {"coding": [{"system": SYS_OBS_CATEGORY, "code": "laboratory"}]}
                ],
                "code": _text(finding.label),
                "subject": patient_ref,
            }
            if doc.date:
                resource["effectiveDateTime"] = doc.date
            ...
            if finding.out_of_range:
                resource["interpretation"] = [
                    {
                        "coding": [
                            {"system": SYS_INTERPRETATION, "code": "A", "display": "Abnormal"}
                        ]
                    }
                ]
    ...
    section_refs = {
        "chief_complaint": condition_refs[:1],
        "past_history": condition_refs[1:],
